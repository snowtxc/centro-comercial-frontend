...
export class EmpresasDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      let idEmpresaParam = this._route.snapshot.paramMap.get('id');
      this.loadEmpresa(idEmpresaParam);
  ...
  loadEmpresa(id_empresa:any) {
    this._empresaService.getEmpresaById(id_empresa).subscribe(empresa => {
  ...
  eventLoadDepartamentos() {this._departamentoService.currentDepartamentosSubject.subscribe(data =>{
